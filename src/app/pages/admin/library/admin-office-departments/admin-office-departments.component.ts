...
import { AdminOfficeDepartmentsAddComponent } from './admin-office-departments-add/admin-office-departments-add.component';

@Component({
  selector: 'fury-admin-office-departments',
  templateUrl: './admin-office-departments.component.html',
  styleUrl: './admin-office-departments.component.scss'
})
export class AdminOfficeDepartmentsComponent {
  pageSize = 10;
  ...
  loading: boolean;
  vesselsFLagFormGroup: UntypedFormGroup;
  ...
  ngOnInit(): void {
    ...
    this.vesselsFLagFormGroup = this.fb.group({
      Department: [""],
      vessel_manager: [""],
      drpStatus: ["1"],
  ...
  clearFilter() {
    this.loading = true;
    this.vesselsFLagFormGroup.reset();
    this.vesselsFLagFormGroup.get('drpStatus').patchValue('1');
    // this.fetchrankCategory();
  }
  ...
  saveVessel(data: object, IsEdit: boolean) {
    const dialogRef = this.dialog.open(AdminOfficeDepartmentsAddComponent, {
      width: '35%',
      maxHeight: '90%',
      disableClose: true,
    });
