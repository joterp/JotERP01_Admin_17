...
import { AdminCitiesAddComponent } from './admin-cities-add/admin-cities-add.component';

@Component({
  selector: 'fury-admin-cities',
...
export class AdminCitiesComponent {
  ...
  constructor(
    ...
    this.titleService.setTitle('Cities');
  ...
  clearFilter() {
    ...
    this.cityFormGroup.get('drpStatus').patchValue('1');
    // this.fetchrankCategory();
  }
  ...
  saveCity(data: object, IsEdit: boolean) {
    const dialogRef = this.dialog.open(AdminCitiesAddComponent, {
      width: '35%',
      maxHeight: '90%',
      disableClose: true,
    });
